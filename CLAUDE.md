...
- **The four LLM documents are build-time `?raw` imports, not `node:fs` reads.**
  `llms.txt` is generated by `docs/src/lib/llms.ts` from the catalogue and
  `package.json`; the other three read `llm.md` and `m3-guidelines.md` through
  Vite. `?raw` rather than `readFileSync` because the root `tsconfig.json` sets
  `types: []` -- deliberately, so the framework's own source cannot reach for
  Node globals -- and a `node:fs` import in a `.ts` endpoint fails `typecheck`
  where the same import in `BaseLayout.astro` does not, since tsc never reads
  `.astro`. `docs/src/env.d.ts` pulls in the `*?raw` declaration.
...
- **`docs/public/CNAME` is the custom-domain declaration, and no `base` is
  ...
  deploy-time URL rewriting `pages.yml` does for the project-subpath address
  unnecessary. Supporting that address is explicitly not a requirement.
