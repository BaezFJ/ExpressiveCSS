...
The public surface is rebranded. Instances are stashed on elements as `el['Expressive_<Component>']`, the IIFE global is `Expressive`, `src/ts/index.ts` exports `version = '0.3.0'` (tracking package.json), and the Materialize-branded markup classes are gone:
...
- `abstracts/_variables.scss` holds the remaining Sass-time knobs (`$root-font-size`, the flow-text bounds, `$font-stack`, `$gutter-width`) — mostly `!default`, several now aliasing CSS custom properties. The heading rem ladder (`$h1-fontsize`…) is unused internally and kept only so downstream Sass overrides still compile.
...
`src/ts/index.ts` re-exports `components/`, `Forms`, `Waves`, `Dialogs`, `AutoInit` and `version`, and runs the import-time side effects: `Forms.Init()`, `Chips.Init()`, `Waves.Init()`, `Range.Init()`, `Cards.Init()`, `Dialogs.Init()`, plus document-level keyboard/focus listeners from `Utils`.
...
- **`Utils.onDocumentReady(fn)`, never a bare `DOMContentLoaded` listener** — the event has already fired when the bundle is loaded async or by dynamic import, and the listener then never runs. All six `Init()` entry points (`Forms`, `Chips`, `Waves`, `Range`, `Cards`, `Dialogs`) go through it.
