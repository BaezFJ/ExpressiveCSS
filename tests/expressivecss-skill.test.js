...
describe('the ExpressiveCSS agent skill', () => {
  ...
  test('lets the generated decision index own every component guide exactly once', () => {
    ...
    assert.equal(componentFiles.length, 41);
  ...
  test('keeps generated component guides synchronized with their sources', () => {
    ...
    assert.match(result.stdout, /41 ExpressiveCSS component guides are current\./);
    ...
    assert.match(mcpResult.stdout, /Verified 41 bundled guides and normative semantics\./);
  });
  ...
  test('bundles the design rules an agent needs before opening a component guide', () => {
    const rulesUrl = new URL('references/design-rules.md', skillDirectory);
    assert.ok(existsSync(rulesUrl), 'design rules reference is missing');
    const rules = readFileSync(rulesUrl, 'utf8');
    assert.match(rules, /^<!-- Generated by scripts\/gen-expressivecss-skill\.mjs\. Do not edit\. -->/);
    for (const heading of ['## Hard rules', '## 1.2 What changes with window size', '## 1.4 Navigation by window size', '## 3. Component chooser', '### 3.3 “How does the product talk back?”', '## 12. Screen recipes', '## 13. Quick anatomy cheat sheet', '## 14. Name map']) {
      assert.ok(rules.includes(heading), `design rules omit ${heading}`);
    }
    // Every copied bullet and table row still exists in its source, so the file cannot drift.
    for (const line of rules.split('\n').filter((entry) => /^[-|]/.test(entry))) {
      assert.ok(m3Guidelines.includes(line), `design rules line is not in m3-guidelines.md: ${line}`);
    }
    assert.match(rules, /Chips come in sets/);
    assert.match(rules, /\| `\.btn`, `\.btn-large`, `\.btn-flat` \| `<button>`/);
    assert.doesNotMatch(rules, /\]\((?:\.\.\/|llm\.md|#)/, 'design rules link outside the portable skill directory');

    assert.match(body, /\*\*Design intent:\*\* the bundled \[design rules\]\(\.\/references\/design-rules\.md\)/);
    assert.match(body, /When no component is named, pick one with the chooser/);
    const design = readFileSync(new URL('expressivecss-design/SKILL.md', skillDirectory), 'utf8');
    assert.match(design, /\[design rules\]\(\.\.\/references\/design-rules\.md\).*component chooser/);
    assert.match(design, /16px on Compact and 24px from Medium/);
    assert.match(design, /--md-comp-pane-margin/);
    assert.match(design, /Button labels are verbs/);
    const usage = readFileSync(usageGuideUrl, 'utf8');
    assert.match(usage, /name map in the \[design rules\]\(\.\.\/references\/design-rules\.md\)/);
    const install = readFileSync(new URL('expressivecss-install/SKILL.md', skillDirectory), 'utf8');
    assert.match(install, /<!doctype html>\n<html lang="en" theme="auto">/);
    assert.match(install, /<meta name="viewport" content="width=device-width, initial-scale=1">/);
    assert.match(install, /`theme` attribute only works on `<html>`/);
  });

  test('inlines JavaScript component options and methods from llm.md', () => {
    const llm = readFileSync(new URL('../llm.md', import.meta.url), 'utf8');
    const guide = (name) => readFileSync(new URL(name, componentsDirectory), 'utf8');
    const tabs = guide('tabs.md');
    assert.match(tabs, /#### Options\n\n\| Name \| Type \| Default \| Description \|/);
  ...
  test('gives agents exact grid and helper-class references', () => {
    ...
    const flexClasses = ['flex', 'inline-flex', 'flex-column', 'flex-wrap', 'justify-start', 'justify-center', 'justify-end', 'justify-between', 'align-start', 'align-center', 'align-end', 'align-stretch', 'flex-1', 'flex-none', ...Array.from({ length: 7 }, (_, i) => `gap-${i}`)];
    assert.match(helpers, /## Flex helpers/);
    assert.match(helpers, /`\.gap-0` to `\.gap-6`/);
    for (const className of flexClasses) {
      if (!className.startsWith('gap-')) assert.ok(helpers.includes(`\`.${className}\``), `helpers reference omits .${className}`);
      assert.match(compiledCss, new RegExp(`\\.${className}(?![\\w-])`), `compiled CSS omits .${className}`);
    }
