...
@app.route('/button-groups')
@app.route('/button-groups.html')
def button_groups():
    return render_template('components/button-groups.html')


@app.route('/split-button')
@app.route('/split-button.html')
def split_button():
    return render_template('components/split-button.html')
...
#
# llms.txt (https://llmstxt.org) is a link index; llms-full.txt is the whole
# corpus in one fetch. Both are routes rather than files copied in by the
# deploy so that `flask run` serves exactly what the published site serves,
# and so Frozen-Flask writes them alongside the pages.
#
# Only `website/llms.txt` is committed. The other three are gitignored under
# `website/`, the same arrangement as `website/dist/`: they are verbatim
# copies of files already in this repository, and committing 290 KB of
# duplicated prose that is rewritten on every docs change buys nothing. The
# freeze still writes them to disk, which is what the deploy copies.

#: Read in this order. `m3-guidelines.md` decides which component a job calls
#: for; `llm.md` states what this framework actually ships.
LLMS_FULL_SOURCES = ('m3-guidelines.md', 'llm.md')


@app.route('/llms.txt')
def llms_txt():
    """The link index, generated by `scripts/gen_llms.py`."""
    return send_from_directory(REPO_ROOT, 'llms.txt', mimetype='text/plain')


@app.route('/llms-full.txt')
def llms_full_txt():
    """Every document a model needs, concatenated.

    Assembled per request rather than generated into a file: there is then no
    third copy to keep in step with the two sources.
    """
    body = '\n\n'.join(
        (REPO_ROOT / name).read_text() for name in LLMS_FULL_SOURCES
    )
    return app.response_class(body, mimetype='text/plain')


def _markdown_source(name):
    return send_from_directory(REPO_ROOT, name, mimetype='text/markdown')


# Two views for two files, not one view with two rules: Frozen-Flask writes one
# file per *endpoint*, so a shared view would freeze whichever URL `url_for`
# happened to build and silently drop the other.
@app.route('/m3-guidelines.md')
def m3_guidelines_md():
    """The design contract, as llms.txt links to it."""
    return _markdown_source('m3-guidelines.md')


@app.route('/llm.md')
def llm_md():
    """The markup and API contract, as llms.txt links to it."""
    return _markdown_source('llm.md')
