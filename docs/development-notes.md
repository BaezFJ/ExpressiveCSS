...
Engineering reference for contributors and agents. Paths in code spans are relative to the repository root. Start with [CONTRIBUTING.md](../CONTRIBUTING.md); release policy lives in [RELEASING.md](../RELEASING.md).
...
Public APIs and markup use ExpressiveCSS names. Elements store instances as `el['Expressive_<Component>']`, the IIFE global is `Expressive`, and `src/ts/index.ts` exports `version = '0.9.1'` to match `package.json`. The renamed classes and APIs are:
...
Framework build uses dart-sass, esbuild, and tsc. Use Node 24 and `npm ci` first.
...
Follow [RELEASING.md](../RELEASING.md). The framework package version and runtime
...
- **Shared document/window listeners need a stable function identity.** ScrollSpy uses a section-owned IntersectionObserver so each offset is honored. Destroy disconnects it, and its callback ignores a destroyed or replaced instance. Sections sharing one `getActiveElement` function share current-link state.
...
Use the canonical triage roles documented in the existing label reference. See `docs/agents/triage-labels.md`.
