...
Public APIs and markup use ExpressiveCSS names. Elements store instances as `el['Expressive_<Component>']`, the IIFE global is `Expressive`, and `src/ts/modular.ts` exports `version = '0.10.0'` to match `package.json`. The renamed classes and APIs are:
