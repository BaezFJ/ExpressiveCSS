...
Unreleased repairs address autocomplete filtering, initial selection and keyboard exit; zero-maximum and resized RTL slider labels; drawer modality at the expanded boundary; snackbar timer suspension during focus or hover; Lightbox Space/native-button activation and teardown; and reduced-motion progress with visible static custom fill. Owning component documentation distinguishes these source repairs from published 0.9.1. Larger gaps remain explicit, including calendar grid keys, time-picker naming, FAB expanded-state/focus behavior, actionable snackbar keyboard behavior, custom progress RTL, and spoken announcement delivery.
