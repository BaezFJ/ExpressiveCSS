...
export async function buildCapabilityRoadmap(data, root) {
  ...
  return { schemaVersion: 1, frameworkVersion: data.frameworkVersion, sourceRevision: data.sourceRevision,
...
export function renderCapabilityRoadmap(roadmap) {
  ...
  const link = (name) => {
    assert.ok(safePath(name), 'Invalid capability link path');
    return `[${name}](https://github.com/BaezFJ/ExpressiveCSS/blob/${roadmap.sourceRevision}/${name})`;
  };
