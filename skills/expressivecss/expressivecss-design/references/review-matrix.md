...
| C-STATE-LAYER-SELECTED | State layers | component + selected state | The selected state uses the container, content, and state treatment documented by that component. | `rendered-capture`, `computed-style`, `source` | `computed-style` | Pass, Intentional adaptation, Fail, Not applicable, Blocked |
| C-STATE-LAYER-DISABLED | State layers | component + disabled state | The disabled state uses the opacity and state treatment documented by that component and paints no interactive state layer. | `rendered-capture`, `computed-style`, `source` | `computed-style` | Pass, Intentional adaptation, Fail, Not applicable, Blocked |
...
Audit rows are hard contracts. `Intentional adaptation` is never an allowed status. Report the Material 48 by 48 dp target recommendation separately from WCAG failures; it is not SC 2.5.8. Measure web targets in CSS pixels regardless of device pixel ratio. For target exceptions, record the applicable exception and evidence instead of inferring it from appearance.
...
| A-FOCUS-ORDER | Focus | task path + input path | Tab and Shift+Tab visit the expected sequential focus stops in meaningful task order, following native or documented component behavior. Group members need not each be a Tab stop; verify internal navigation separately with A-KEYBOARD-INTERNAL. | `interaction-trace`, `accessibility-tree` | `interaction-trace` | Pass, Fail, Not applicable, Blocked |
...
| A-STATUS-SEMANTICS | Status semantics | dynamic event | The message has the appropriate status/live-region semantics and update timing for its intended notification without moving focus. Mere text presence in the tree is insufficient. | `source`, `accessibility-tree`, `interaction-trace` | `accessibility-tree`, `interaction-trace` | Pass, Fail, Not applicable, Blocked |
| A-ANNOUNCEMENT | Announcements | dynamic event + assistive path | An interaction trace from the named browser, OS, and assistive technology records delivery of the intended message after the event, without unwanted duplicates or focus movement. Tree inspection alone cannot pass. | `interaction-trace` | `interaction-trace` | Pass, Fail, Not applicable, Blocked |
| A-KEYBOARD-INTERNAL | Keyboard behavior | group + key + outcome | Component-internal keys reach and operate the expected items according to the native or documented keyboard model, including radio-group arrow navigation. | `interaction-trace` | `interaction-trace` | Pass, Fail, Not applicable, Blocked |
| A-HOVER-CONTENT | Hover/focus content | popup + input path + condition | Each applicable SC 1.4.13 condition, dismissible, hoverable, or persistent, passes the focused web checks. Create one row per condition and record any applicable exception. | `interaction-trace`, `rendered-capture` | `interaction-trace` | Pass, Fail, Not applicable, Blocked |
| A-CONTRAST-NORMAL | Contrast | normal-text sample | For text covered by SC 1.4.3, the measured effective foreground/background pair is at least 4.5:1 in the declared theme and state; record any applicable exception. | `contrast-measurement`, `computed-style` | `contrast-measurement` | Pass, Fail, Not applicable, Blocked |
| A-CONTRAST-LARGE | Contrast | large-text sample | For text covered by SC 1.4.3, at least 18 pt or 14 pt bold, the measured effective foreground/background pair is at least 3:1 in the declared theme and state; record any applicable exception. | `contrast-measurement`, `computed-style` | `contrast-measurement` | Pass, Fail, Not applicable, Blocked |
| A-CONTRAST-NONTEXT | Contrast | non-text sample | Visual information needed to identify an authored control, state, or meaningful graphic meets SC 1.4.11 at 3:1 against adjacent colors in the declared theme and state. Record inactive-control or unmodified-user-agent exceptions for controls, and essential-presentation exceptions for graphics, when applicable. | `contrast-measurement`, `computed-style` | `contrast-measurement` | Pass, Fail, Not applicable, Blocked |
...
| A-TOUCH-TARGET | Touch targets | control | The active pointer target is at least 24 by 24 CSS pixels under WCAG 2.2 AA SC 2.5.8, or evidence establishes a spacing, equivalent, inline, user-agent-control, or essential exception. For spacing, centered 24 CSS pixel diameter circles on undersized target bounding boxes intersect neither another target nor another undersized target circle. | `runtime-inspection`, `test-output` | `runtime-inspection` | Pass, Fail, Not applicable, Blocked |
| A-DRAG-POINTER | Dragging | drag action + outcome | Under SC 2.5.7, the same outcome is reachable with a single pointer without dragging, or evidence establishes essential dragging or unmodified user-agent functionality. Keyboard operation is checked independently. | `interaction-trace`, `source` | `interaction-trace` | Pass, Fail, Not applicable, Blocked |
| A-FORCED-COLORS | Forced colors | control + state | With forced colors active, the fixture-declared control boundary, state, or focus indicator remains visible and the action remains operable. Create a separate row for each fact; record browser and whether the mode is emulated. | `rendered-capture`, `interaction-trace`, `computed-style` | `rendered-capture`, `interaction-trace` | Pass, Fail, Not applicable, Blocked |
...
| A-REDUCED-MOTION | Reduced motion | motion preference + task path + one motion fact | With reduced motion enabled, the one fixture-declared task-completion or state-visibility fact passes. Create separate rows for completion and each required state change. Also check the actual motion treatment with A-REDUCED-MOTION-TREATMENT. | `interaction-trace`, `rendered-capture` | `interaction-trace`, `rendered-capture` | Pass, Fail, Not applicable, Blocked |
| A-REDUCED-MOTION-TREATMENT | Reduced motion | transition + motion preference | Compared with the default preference, the spatial or repetitive motion identified in the brief is reduced or suppressed while useful feedback remains. Record the observed change, or why no such motion exists. This is the skill motion requirement, not a blanket WCAG AA criterion. | `interaction-trace`, `computed-style` | `interaction-trace` | Pass, Fail, Not applicable, Blocked |
| A-TEXT-SPACING | Text spacing | affected text + override | Under SC 1.4.12 spacing overrides, content and controls remain available without clipping or loss of function; record applicable script exceptions. | `rendered-capture`, `interaction-trace` | `rendered-capture`, `interaction-trace` | Pass, Fail, Not applicable, Blocked |
| A-FOCUS-OBSCURED | Focus obscuration | focused control + scroll state | Under SC 2.4.11, author-created content does not entirely hide the focused control; apply the criterion's user-positioned-content and user-opened-content notes where relevant. | `rendered-capture`, `interaction-trace` | `rendered-capture`, `interaction-trace` | Pass, Fail, Not applicable, Blocked |
| A-AUTHENTICATION | Authentication | authentication step | Under SC 3.3.8, any cognitive-function test has a permitted mechanism, alternative, or exception. Password-manager and paste support are tested where they provide the mechanism. | `interaction-trace`, `source` | `interaction-trace` | Pass, Fail, Not applicable, Blocked |
