...
| A-ANATOMY | Anatomy | component | Every required child exists once in the documented order and nesting. | `source`, `runtime-inspection` | `source` | Pass, Fail, Not applicable, Blocked |
| A-LABEL | Labels | control or region | The accessible name equals the fixture-required label and names the intended control or region. | `source`, `accessibility-tree` | `accessibility-tree` | Pass, Fail, Not applicable, Blocked |
...
| A-KEYBOARD-ACTION | Keyboard behavior | action | The documented keyboard input invokes the action once and reaches the same outcome as pointer input. | `interaction-trace`, `test-output` | `interaction-trace` | Pass, Fail, Not applicable, Blocked |
...
| A-ZOOM-200 | Zoom | route + task point | At 200 percent text resizing, all required content and actions remain present and operable. | `rendered-capture`, `interaction-trace` | `rendered-capture`, `interaction-trace` | Pass, Fail, Not applicable, Blocked |
| A-REFLOW-320 | Reflow | route + task point | At a 320 CSS px-equivalent viewport, applicable content has no two-dimensional scrolling and the task remains operable. | `rendered-capture`, `interaction-trace` | `rendered-capture`, `interaction-trace` | Pass, Fail, Not applicable, Blocked |
...
| A-REDUCED-MOTION | Reduced motion | motion preference + task path | With reduced motion enabled, the task completes and no required state change is hidden by removed animation. | `interaction-trace`, `rendered-capture` | `interaction-trace` | Pass, Fail, Not applicable, Blocked |
...
Use the generated guide's `Component ID` value and an exact rule ID printed under its `Rules` heading. The final row below demonstrates the join with the real `buttons` component and its real `icon-only-control-is-named` semantics rule. Copy or link each applicable generated-guide rule as one row.

| Criterion instance ID | Component ID | Criterion ID | Exact target-version contract fact | Status | Evidence record ID |
| --- | --- | --- | --- | --- | --- |
| `<component-id>@A-HOST-ELEMENT` | `<component-id>` | `A-HOST-ELEMENT` | Exact documented host tag. | — | — |
| `<component-id>@A-ANATOMY` | `<component-id>` | `A-ANATOMY` | One required child, order, or nesting fact per row. | — | — |
| `<component-id>@A-LABEL` | `<component-id>` | `A-LABEL` | One label and named target per row. | — | — |
...
| `buttons@icon-only-control-is-named` | `buttons` | `icon-only-control-is-named` | An icon-only control has an accessible name after its decorative icon is hidden. | — | — |
